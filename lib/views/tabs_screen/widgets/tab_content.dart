...
class _TabContentState extends State<TabContent> {
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (picked != null) {
      setState(() {
        booking.searchData.departureDate = "${picked.year}-${picked.month}-${picked.day}";
      });
    }
  }
  ...
  try {
    formattedDate = DateFormat('yyyy-MM-dd').format(DateFormat('yyyy-M-d').parse(booking.searchData.departureDate!));
  } catch (e) {
                      ...
                      onChanged: (newValue) {
                        setState(() {
                          if (title == 'Departure From') {
                            ...
                            bookingProvider.searchData.departureFromId = cityMap[newValue]!;
                            bookingProvider.searchData.departureStation = newValue;
                          } else {
                            selectedArrival = newValue;
                            bookingProvider.searchData.arrivalToId = cityMap[newValue]!;
                            bookingProvider.searchData.arrivalStation = newValue;
                          }
                        });
                      },
                    )
                ...
                IconButton(
                  onPressed: () {
                    if (booking.searchData.travelers! > 1) {
                      setState(() {
                        booking.searchData.travelers = booking.searchData.travelers! - 1;
                      });
                    }
                  },
                  icon: const Icon(Icons.remove_circle, color: Colors.grey),
                ),
                Text('${booking.searchData.travelers!}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                IconButton(
                  onPressed: (){
                    setState(() {
                      booking.searchData.travelers = booking.searchData.travelers! + 1;
